...
class octothorpe:
    ...
    copy = lambda self: octothorpe(_cache=self._cache[:], _length=self._length, _state=self._state[:], _history=self._history[:])
    digest = lambda self: bytes(self._finalize())
    hexdigest = lambda self: self.digest().hex()

    def __init__(self, data: bytes = None, *, _cache: bytes = None, _length: int = None, _state: bytearray = None, _history = None) -> None:
        self._cache = _cache or b''
        self._length = _length or 0
        self._state = _state or self.initial_state[:]
        self._history = _history or b''
    ...
    def _finalize(self) -> bytearray:
        ...
        print(f"hist_len: {len(clone._history)}, history: {clone._history}")
        return clone._state
